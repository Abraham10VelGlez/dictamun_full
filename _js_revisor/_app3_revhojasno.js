...
$(document).ready(function () {
    ...
    //Total de inmuebles a revisar
     $.post("../../acceso",{acceess:222143,idI:idI, anio:anioxc},
    	function(n){
        ...
        var conta=0;
         var contaTipo=0;
            $(z).each(function(key,valuee){
              ...
              filesavg(n,idI);
              files_tipologias(n,idI);
              infor_data_pre(n,idI);
              prediometodo_view_data(n,idI);
              load_valterr_avg(n,idI);
      var fol2 = $("#cc").val();
      var inicio = "DIP/";
      var inicio2 = "DIP/";
   ...
   $("#descargarHojasVerdes").append(' <center><label>Archivo en hojas verdes&nbsp;&nbsp;&nbsp;&nbsp;</label><a id="descargarActaC" href="https://dictamunigecem.edomex.gob.mx/files/documentos/'+fol2+'/'+clavC+'/dic_hojasverdes.pdf" download="dic_hojasverdes.pdf"><i class="fa fa-download fa-2x"></i></a></center>')
          ...
          document.getElementById('alertaa').style.display='block';
             ...
             if (conta == 1 ) {
                ...
                observacionesgeneralesxfolioclav(n,idI);

               // observacionGralPagos
...
function files_tipologias(f,c){
  ...
  $.ajax({
    ...
    success: function(datar)
    {
      ...
      if( datar == "50" ){
        //console.log("no hay tipologias disponibles, no cargo archivo de tipologias");
...
function infor_data_pre(f,c){
  var data = new FormData();
  data.append('acceess',"449947");
  data.append('folio',f);
  data.append('cllv',c);
  var time_count = 0;
  $.ajax({
    ...
    success: function(datar)
    {
      ...
      $.each( datar, function( key, valuee ) {
       ...
       var valor_terr = new Intl.NumberFormat('es-MX').format(valuee.sumavalorterravg);
...
function observacionesgeneralesxfolioclav(f,c){

  var comm = "";
  var data = new FormData();
  data.append('acceess',"4499413");
  data.append('folio',f);
  data.append('cllv',c);
$.ajax({
url: "../../acceso",
...
function prediometodo_view_data(folioD,clavecatastral){
...
var valor_redon=0;
  ...
  var pocientoConTotal2=0;
          ...
          var data = new FormData();
    data.append('acceess',"38");
    data.append('folioD',folioD);
    data.append('clavecatastral',clavecatastral);
...
/*
        if (valuee.tipoconstruccion == 1) {
          ...
          valor_comu = new Intl.NumberFormat('es-MX').format(comun);
           //$("#comun").val(valor_comu);
        ...
        porcientoCon = porcentajeCondominio2 / 100;
